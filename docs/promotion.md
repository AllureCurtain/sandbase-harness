...
- [Plugin Hub duplicate-install issue #78](https://github.com/sandbaseai/sandbase-harness/issues/78): open; added a non-destructive recovery path (update Hub, remove only the exact residual target, retry the fixed HTTPS Git source) and requested the pre-error log context for an upstream idempotency fix ([comment](https://github.com/sandbaseai/sandbase-harness/issues/78#issuecomment-5468721664)).
