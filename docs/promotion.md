...
- [TensorBlock Awesome MCP Servers issue #2067](https://github.com/TensorBlock/awesome-mcp-servers/issues/2067): submitted a separate SandBase Harness MCP entry through the active server-submission queue, with the six real tools, stdio Docker install command, Apache-2.0 license, and supported-client metadata. This is intentionally separate from the existing SandBase CLI issue; curator review is pending.
