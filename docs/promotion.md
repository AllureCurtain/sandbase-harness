...
- [MCP Atlas PR #2](https://github.com/DTennant/mcp-atlas/pull/2): open and mergeable; added SandBase Harness to the searchable MCP catalog using the official MCP Registry as the source, with JSON validation passing.
...
- [Awesome Agent Runtime Security PR #28](https://github.com/bureado/awesome-agent-runtime-security/pull/28): open and mergeable; SandBase Harness added to Provenance, Instrumentation & Observability with conservative MCP, credential, policy, audit/replay, and backend-dependent isolation wording.
- [Awesome Agent Security PR #11](https://github.com/authora-dev/awesome-agent-security/pull/11): open and mergeable; SandBase Harness added to Sandboxed Execution with concise, backend-aware wording for MCP governance, credentials, approvals, and audit/replay.
- [Awesome Agent Observability PR #11](https://github.com/danielt69/awesome-agent-observability/pull/11): open and mergeable; SandBase Harness added to Gateways with Built-in Observability with a conservative description of its MCP gateway, controls, and audit/replay records.
- [Adventure Wave Awesome Agent Security PR #2](https://github.com/adventurewave-labs/awesome-agent-security/pull/2): open and mergeable; SandBase Harness added to Runtime Guardrails with MCP governance, approvals, credential controls, audit/replay, and backend-aware isolation wording.
- [Yanmxa Awesome Agent Sandbox PR #1](https://github.com/yanmxa/awesome-agent-sandbox/pull/1): open and mergeable; SandBase Harness added to Standalone Sandboxes with the list's live Stars badge, `any runtime` isolation label, and backend-aware wording. GitGuardian check is still in progress.
- [IronSecCo Awesome Agent Sandboxing PR #1](https://github.com/IronSecCo/awesome-agent-sandboxing/pull/1): open and mergeable; SandBase Harness added to Self-hosted agent runtimes with Apache-2.0 and backend-aware wording. Local `awesome-lint` reports pre-existing repository errors (duplicate links, ToC/license issues, and missing punctuation) outside this one-line addition.
...
- [AIM-Intelligence Awesome MCP Security PR #49](https://github.com/AIM-Intelligence/awesome-mcp-security/pull/49): open and mergeable; SandBase Harness added to Tools with MCP bridge, approval policy, credential, sandbox-session, and audit/replay wording.
...
- [Awesome MCP Registry Servers PR #5](https://github.com/formulahendry/awesome-mcp-registry-servers/pull/5): open and mergeable; added the official v0.3.8 SandBase Harness OCI/stdio metadata with required runtime URL and optional API-key variables. JSON validation passed.
...
- [mcpHQ Awesome MCP Servers PR #45](https://github.com/mcpHQ/awesome-mcp-servers/pull/45): open; added a separate SandBase Harness entry alongside the existing SandBase CLI entry in the catalog. `npm run validate`, `npm run generate`, and `npm run check-generated` passed for 138 servers across 13 categories.
- [DhanushNehru Awesome MCP Servers PR #75](https://github.com/DhanushNehru/awesome-mcp-servers/pull/75): open; added SandBase Harness to Development & DevOps with a concise, factual description of its MCP bridge, sandboxed turns, artifacts, audit, and replay. The target list had no existing Harness entry.
...
- [MrPeppersDev Agent Infrastructure Landscape intake #173](https://github.com/MrPeppersDev/agent-infrastructure-landscape/issues/173): submitted SandBase Harness to the catalog's documented auto-research intake for `AI sandbox & runtime environments`, including v0.3.8, Apache-2.0, official Registry, and backend-aware isolation notes. The Issue was created successfully; the repository currently has no `intake` label, so no label is claimed.
