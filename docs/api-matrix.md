...
| Webhooks | `/v1/webhooks` | Advanced | Create/list/update/archive, test deliveries, delivery attempts, and retry due deliveries. A first attempt carries the Standard Webhooks v1 headers, with the signature covering id, timestamp, and body, constant-time verification, and a rotation window. Dispatch and retry are on-demand calls: there is no background dispatcher, and a non-2xx is retried rather than disabling the endpoint. Not needed for the first local run. |
