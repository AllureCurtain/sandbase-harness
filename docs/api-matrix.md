...
| Sessions | `/v1/sessions` | Supported | Create, list, retrieve, stop, delete, durable event ingestion/listing, sequence-resumable SSE, and message convenience endpoint A stream connection can opt into token-level previews with a repeated `event_deltas[]` parameter for `agent.message` and `agent.thinking`; preview frames carry no id and are never persisted. | Creation also accepts an optional `initial_events` array of `user.message` and `user.define_outcome` events that are validated and written in the same transaction, so a non-empty list yields a `running` session whose log already holds them and a rejected batch leaves no session behind. A `user.define_outcome` event carries the session's success criteria — a `description`, a `rubric` (inline text or a file reference) and a `max_iterations` budget that defaults to 3 and is capped at 20 — and its payload is projected back onto the event listing; the same normalization and its `invalid_define_outcome` refusal apply to a live `POST /v1/sessions/{id}/events`. Create accepts optional `loop_engine`; the resolved engine is persisted and frozen on the session. Unavailable engines return `loop_engine_not_supported`, unknown values return `loop_engine_invalid`, and rejected requests leave no session row or event. Session status includes `requires_action` while an approval group is pending. Event envelopes include `seq`, immutable metadata, and optional model/token/stop-reason/duration metadata. A `session.usage` snapshot event carrying `input_tokens`, `output_tokens`, and `active_seconds` is written immediately before every `session.status_idle`; it also carries `list_cost` when every model the session used has a configured list price, the session's `budget` or `null`, and zeroed `server_tool_use` counters, because this runtime has no built-in web tool. Creation also accepts an optional `budget` (`{type: "limit", max_list_cost: {amount, currency: "USD"}}`) priced from an operator-supplied cost profile; a session whose model has no list price is refused a budget with `budget_model_without_list_price` rather than metered against an invented rate, and once the ceiling is reached a work-starting event is refused with `budget_reached` while events that settle in-flight work are still accepted. A declared outcome stops at the ceiling too, closing with `result: "budget_reached"` rather than starting another grading pass or turn. `agent.mcp_tool_use` and `agent.mcp_tool_result` events carry `mcp_server_name` (and results carry `mcp_tool_use_id`) so two servers exposing the same tool name stay distinguishable. Pi stdout final replies and native tool trajectories use the same event history; transient deltas are not replayed; Pi native tools remain outside Harness approval and sandbox path policy, and a native tool declared `always_ask` is gated by the session's own managed Pi extension before it executes, with one durable pending record per call that only a single decision can consume. Pi cleanup may surface `cancelled`, `timed_out`, or `cleanup_pending` rather than fabricated success. | Creation validates web-tool domain lists on the supplied agent definition and rejects a malformed list before the session is persisted. | Creation also accepts the `agent_with_overrides` reference form, which runs a pinned or current agent version with any of `model`, `system`, `tools`, `mcp_servers`, or `skills` replaced for that session only under the tri-state rule: an omitted field is inherited, `null` (or `[]` for a list) clears it, and a value replaces it wholesale. The resolved definition is frozen as the session's own snapshot and is what capability admission judges, while the agent row and its version list are untouched. A refusal is a 400 raised before the session row exists, carrying `agent_model_required`, `agent_tools_cleared_with_skills`, `agent_mcp_server_not_found`, `invalid_agent_override_field`, `invalid_agent_overrides`, or `invalid_agent_ref`. |
| Runs | `/v1/runs` | Supported | Start one turn and return its result. `response_mode` selects `wait` (200 with output and usage), `sse` (event stream), or `async` (202 with a query handle); `max_wait_seconds` bounds only the wait and answers 202 with `wait_deadline_reached` when it elapses. Pre-execution refusals answer with their own status, and a mid-run failure is recorded once as `session.error`. Session budgets are not part of this endpoint. `agent` accepts only the id string and the pinned object form; the `agent_with_overrides` form is refused with a message naming the reason rather than accepted and ignored. |
...
| Outcomes | `/v1/outcomes` and `/v1/sessions/{id}/outcomes` | Advanced | Create/list/update/archive outcomes and evaluate sessions. Not needed for the first local run. | A session's own declared outcome (`user.define_outcome`) is separate from these records: it projects into the turn it queues and, once that turn completes, the runtime appends `span.outcome_evaluation_start`, `span.outcome_evaluation_ongoing` and `span.outcome_evaluation_end` to the session log, the last carrying `result` (`satisfied | needs_revision | failed | max_iterations_reached | interrupted | budget_reached`) and an explanation. A `needs_revision` verdict drives its own revision loop: the explanation is appended as a real `user.message` and another turn runs in the same outcome, bounded by the declared `max_iterations`, whose last allowed evaluation reports `max_iterations_reached` and still leaves the agent one final turn to settle its answer; an interrupt closes the outcome as `interrupted` with no `session.error`, and a session that spends its declared `budget` ceiling closes it as `budget_reached` without starting another grading pass or turn. Grading needs a model provider; with none configured the evaluation closes as `failed` and the session records `outcome_evaluator_unavailable` with `retry_status: not_retryable`, and a runtime that composes no grader refuses the declaration at admission with `outcome_grader_unavailable`. |
