...
export class PiStrategy implements AgentStrategy {
  ...
  async *execute(context: StrategyContext) {
    ...
    const request: PiLaunchRequest = {
      ...
      // The same compiler admission ran: whatever passed there is what is sent.
 ...
 *
 * A Pi session freezes its own agent definition when it is created, and
 * admission compiled that same definition, so this is the definition the flags
 * come from. There is deliberately no fallback: without it the runtime cannot
 * say which tool set it is allowed to expose, and guessing would be the widening
