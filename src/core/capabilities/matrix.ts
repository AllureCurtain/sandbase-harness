...
export const CMA_CAPABILITY_MATRIX: readonly CapabilityEntry[] = [
  ...
  },
  {
    ...
    id: 'opaque-cursors',
    status: 'partial',
    reason: 'A collection\'s envelope follows the mount: the operations router serves `/v1` with `{data, prev_page, next_page}` and its `/v1/x` mirror with the local `{data, has_more, first_id, last_id}`, chosen through one pager so no handler emits both spellings, and cursors that are readable base64url JSON rather than opaque binary. They are `null` on a complete result set, which is why the Vault, credential, memory-store, memory, agent, agent-version, api-key, environment, worker-key and file listings carry `{data, prev_page: null, next_page: null}`, and a windowed page is addressed by an offset rather than by a stable sort key: `/v1/skills` and the credential audit listings carry a followable `{offset, filter}` cursor, so a cut page says so and a cursor issued for another filter is refused. `/v1/sessions` is the only canonical collection still on the local envelope, and the work-item listing is a windowed extension shape with a `counts` object; both are named in the contract.',
