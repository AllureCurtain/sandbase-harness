...
export const CMA_CAPABILITY_MATRIX: readonly CapabilityEntry[] = [
  ...
  },
  {
    ...
    id: 'opaque-cursors',
    status: 'partial',
    reason: 'A collection\'s envelope follows the mount: the operations router serves `/v1` with `{data, prev_page, next_page}` and its `/v1/x` mirror with the local `{data, has_more, first_id, last_id}`, chosen through one pager so no handler emits both spellings, and cursors that are readable base64url JSON rather than opaque binary. They are `null` on a complete result set, which is why the Vault, credential, memory-store and memory listings carry `{data, prev_page: null, next_page: null}`, and a windowed page is addressed by page number rather than by a stable sort key. Two collections still mix or cannot describe themselves: `/v1/skills` returns the local fields and a `next_page` in one body, and the credential audit listings truncate by `limit` with nothing to continue from. The SDK-typed collections (`agents`, `api-keys`, `environments`, `files`, `sessions`) return the local envelope, which the contract records as a difference and which is converted collection by collection because the SDK and the Console read that shape.',
