...
Two disable rules are implemented. The first published one: an attempt that observes a
...
see §4 for why that default is a recorded deviation rather than an oversight. The
remaining published rules — the sustained-failure window and reset-on-success — have no
implementation here, and neither does the published 5–120 s jitter.
