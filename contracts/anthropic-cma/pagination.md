...
| Two version listings are unwindowed although the published contract paginates them | `/v1/agents/{id}/versions` and `/v1/memory_stores/{id}/memory_versions` both return their whole set with `next_page: null` and `prev_page: null`, and neither implements a window. The published contract paginates both — the documented clients walk them with the SDK's `autoPager()` (`智能体设置.md`, `记忆存储.md`) — and documents `limit` plus a `page` cursor carrying the `next_page` value as the convention (`会话操作.md`). The two differ locally in how they treat those parameters: `memory_versions` **refuses** them, accepting `memory_id` only, so a client that tries to paginate is told rather than misled, while the agent version history does not read them yet and answers its whole set, so a client that paginates it receives every version in one response and stops. |
