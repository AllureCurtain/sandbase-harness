...
| Coverage of the codes | `tests/unit/error-code-coverage.test.ts` pins which test files mention each code, against `tests/fixtures/error-code-coverage.json`. Measured, **10 of the 58 codes are mentioned by no test** (`invalid_json`, `already_exists`, the `pi_rpc_*` family, `model_auth_failed`, `outcome_rubric_file_not_found`, `pi_policy_mismatch`). Those are a **recorded gap, not a passing grade**: the fixture keeps the number visible and the test fails if a code silently gains or loses its only assertion. Coverage here means literal presence in the test tree, not execution. |
